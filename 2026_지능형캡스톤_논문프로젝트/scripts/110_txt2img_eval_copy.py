...
# ===== 6. 평가 및 융합 (Feature-level Fusion & Grid Search) =====
...
def get_aligned_gen_features(q_f, q_p, q_c, g_p, g_c, gen_f, gen_p, gen_c):
    """쿼리(q_f)와 동일한 순서/크기를 가지는 생성본 피처 맵을 만듭니다."""
    aligned_gen_f = np.zeros_like(q_f)
    has_gen_mask = np.zeros(len(q_f), dtype=bool)
    
    # 쿼리의 시점(c1~c5)과 ID에 맞는 생성본(c6 포즈로 변환된) 피처를 매칭
...
logline("\n" + "=" * 75)
logline(f"## [2026-06-03] Feature-level Fusion & Alpha Grid Search")
logline(f"   - Baseline (B0) Rank-1: {r1_base:.1f} | mAP: {map_base:.1f}")
logline("=" * 75)

# 열화된 생성본(gen_matched)으로 테스트
gen_aligned_f, valid_mask = get_aligned_gen_features(q_f, q_p, q_c, g_p, g_c, mat_f, mat_p, mat_c)

# 0.5부터 1.0까지 0.05 단위로 Alpha 조절하며 최적점 탐색
logline(f"{'Alpha (원본 비중)':<18}{'Rank-1':<10}{'mAP':<12}{'Diff (mAP)':<10}")
logline("-" * 55)

best_map = 0
best_alpha = 1.0

# numpy array를 다루기 편하게 정규화 함수
def normalize(v):
...
for alpha in np.arange(0.5, 1.01, 0.05):
    ...
    # Feature-level Fusion (벡터 공간에서 융합)
    fused_q_f = q_f.copy()
    
    # 생성본이 존재하는 데이터만 융합 적용
    fused_q_f[valid_mask] = (alpha * q_f[valid_mask]) + ((1.0 - alpha) * gen_aligned_f[valid_mask])
    fused_q_f = normalize(fused_q_f) # 더한 후 다시 정규화 (매우 중요!)
    
    # 융합된 피처로 거리 계산
    fused_dist = cosd(fused_q_f, g_f)
    r1, mAP = eval_market(fused_dist, q_p, g_p, q_c, g_c)
    r1 *= 100; mAP *= 100
    ...
    logline(f"{alpha:<18.2f}{r1:<10.1f}{mAP:<12.1f} ({sign}{diff:.2f})")

logline("-" * 55)
logline(f"🎯 결론: 최적의 성능은 Alpha = {best_alpha:.2f} 일 때 mAP {best_map:.1f} 입니다.")