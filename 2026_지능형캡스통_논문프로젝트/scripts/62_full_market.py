...
"""
Cross-Domain 전체 평가
Source: Duke 학습 CLIP-ReID
Target: Market-1501 (모든 가능한 ID)

Oracle 분석 + Confidence 비교
"""
...
GEN_BASE = f"{PROJECT_DIR}/outputs/c1base_gen_all"  # 기존 100명 + 추가
...
# === 평가 ===
print("\n" + "="*70)
print("STEP 2: Duke 학습 → Market 평가 (Cross-Domain)")
print("="*70)
...
print("\n" + "="*100)
print(f"Cross-Domain 전체 Oracle 분석 (Duke 학습 → Market 평가)")
print("="*100)
print(f"\n{'Pair':<10}{'N':<6}{'BL맞':<8}{'BL틀':<8}{'TopK내':<10}{'회복':<8}{'Oracle맞':<12}{'BL%':<10}{'Oracle%':<12}{'향상':<10}")
print("-"*100)
...
for cam in TARGET_CAMS:
    ...
    print(f"c1→{cam:<7}{r['N']:<6}{r['BL_correct']:<8}{r['BL_wrong']:<8}"
          ...
          f"{bl:<10.2f}{oracle:<12.2f}{gain:+.2f} {mark}")
    for k in r:
        total[k] += r[k]
...
print(f"{'합계':<10}{total['N']:<6}{total['BL_correct']:<8}{total['BL_wrong']:<8}"
      ...
      f"{bl_avg:<10.2f}{oracle_avg:<12.2f}{gain_avg:+.2f}")
...
print(f"\n[Cross-Domain 종합]")
print(f"100명 (이전): Oracle +2.48%p")
print(f"전체 (지금):  Oracle {gain_avg:+.2f}%p")
